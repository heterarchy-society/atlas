...
async function buildGitIndexNative(dir, sourceDir, ext, { dirBased = false } = {}) {
  ...
  for (const { meta, changes } of parseGitNameStatusLog(out)) {
...
async function buildGitIndexIsomorphic(dir, sourceDir, ext, { dirBased = false } = {}) {
  const dataGlob = dirBased ? `${sourceDir}/*/index.${ext}` : `${sourceDir}/*.${ext}`
  const index = new Map()
  const changelog = []
