...
function gitLog(rootDir, glob) {
  try {
    return execSync(
      `git log --format="COMMIT\x1f%H\x1f%aI\x1f%an\x1f%ae\x1f%s" --name-status -- "${glob}"`,
...
function buildGitIndex(rootDir, sourceDir, ext) {
  const glob = `${sourceDir}/*.${ext}`
  const out = gitLog(rootDir, glob)
  const index = new Map()
  const changelog = []
  ...
  for (const line of out.split('\n')) {
    if (line.startsWith('COMMIT\x1f')) {
      ...
      const id = filename.slice(sourceDir.length + 1).replace(/\.\w+$/, '')
...
export function buildHistory(items, rootDir, sourceDir, ext, distDir) {
  const historyDir = join(distDir, 'history')
  mkdirSync(historyDir, { recursive: true })

  const { index, changelog } = buildGitIndex(rootDir, sourceDir, ext)
  const authorCache = loadAuthorCache(rootDir)

  for (const item of items) {
    const filename = `${sourceDir}/${item.id}.${ext}`
    ...
    if (seen.size > 0) item.authors = [...seen.values()]
