import { readFileSync, readdirSync, writeFileSync, mkdirSync, existsSync, copyFileSync, statSync, unlinkSync } from 'node:fs'
...
import { readConfig, readFirstCollection, buildBundleConfig } from './config.js'
...
export async function build() {
  ...
  const index = {
    meta: {
      ...
      config: buildBundleConfig(config),
      atlas: {
  ...
  writeBundle(DIST_DIR)
}

const BUNDLE_NAME = 'bundle.tar.zst'
const BUNDLE_TMP_TAR = '.bundle.tmp.tar'

function writeBundle(distDir) {
  const bundlePath = join(distDir, BUNDLE_NAME)
  const tmpTar = join(distDir, BUNDLE_TMP_TAR)
  ...
  execFileSync('tar', [
    '-cf', tmpTar,
    '--exclude', BUNDLE_NAME,
    '--exclude', BUNDLE_TMP_TAR,
    '-C', distDir,
    '.',
  ])

  const compressed = zstdCompressSync(readFileSync(tmpTar))
  writeFileSync(bundlePath, compressed)
  unlinkSync(tmpTar)

  const { size } = statSync(bundlePath)
  const mb = (size / (1024 * 1024)).toFixed(2)
  console.log(`Written: dist/${BUNDLE_NAME} (${mb} MB)`)
}
