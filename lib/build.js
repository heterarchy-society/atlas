...
function collectionLatestCommit(rootDir, sourceDir, format) {
  const ext = format === 'yaml' ? 'yaml' : format
  try {
    const out = execSync(
      `git log -1 --format="%H%x1f%aI" -- "${sourceDir}/*.${ext}"`,
...
export async function build() {
  ...
  for (const col of config.collections) {
    ...
    index.meta[outputKey] = {
      ...
      latestCommit: collectionLatestCommit(ROOT, col.source_dir, col.format),
    }
