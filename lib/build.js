...
async function collectionLatestCommit(rootDir, sourceDir, format, dirBased) {
  ...
  for (const col of config.collections) {
    ...
    if (col.copy_sources && col.dir_based) {
      ...
      for (const item of transformed) {
        ...
        const cacheSrc = join(ROOT, '.atlas-cache', col.source_dir, item.id)
        if (existsSync(cacheSrc)) {
          copyDirSources(cacheSrc, join(DIST_DIR, outputKey, item.id), indexFile)
        }
      }
    ...
    index.meta[outputKey] = {
      ...
      latestCommit: await collectionLatestCommit(ROOT, col.source_dir, col.format, col.dir_based),
    }
